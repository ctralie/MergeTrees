...
def mainExample():
    ...
    (MT2, PD2) = mergeTreeFrom1DTimeSeries(-x)
    ...
    (MT1Dict, PD1) = mergeTreeFrom1DTimeSeries(x1)
    ...
    (MT1Dict, PD) = mergeTreeFrom1DTimeSeries(x1)
    ...
    ax = plt.gca()
    ax.set_yticks(yvals)
    ax.set_yticklabels(ystrs, fontsize=24)
    ax.set_xticks([])
    plt.grid()
...
#    plt.plot(PD4[:, 0], PD4[:, 1], '.')
    ...
    ax = plt.gca()
    ax.set_xticks(PD1[:, 0])
    ax.set_xticklabels([d[i] for i in PD1[:, 0]])
    ax.set_yticks(PD1[:, 1])
    ax.set_yticklabels([d[i] for i in PD1[:, 1]])
    plotDGM(PD1, sz=40)
    plt.grid()
    plt.axis('equal')
